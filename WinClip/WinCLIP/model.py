...
class WinClipAD(torch.nn.Module):
    def __init__(self, out_size_h, out_size_w, device, backbone, pretrained_dataset, scales, precision='fp32', **kwargs):
        ...
        # Initialize FrequencyEncoder (yslin)
        sys.path.append(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))
        from FrequencyEncoder import FrequencyEncoder
        self.freq_encoder = FrequencyEncoder(in_channels=3, out_dim=640).to(device)
    ...
    def encode_image(self, image: torch.Tensor):
        ...
        # Frequency features from FrequencyEncoder (yslin)
        freq_features = self.freq_encoder(image.float())  # (B, 640)
        freq_features = freq_features / freq_features.norm(dim=-1, keepdim=True)

        # Fuse spatial and frequency features for each scale
        fused_features = []
        for f in image_features:
            fused = f.float() + freq_features
    ...
    def calculate_textual_anomaly_score(self, visual_features):
        ...
        for indx, (features, mask) in enumerate(zip(visual_features, self.masks)):
            normality_and_abnormality_score = (100.0 * features.float() @ self.text_features.float().T).softmax(dim=-1)
            normality_score = normality_and_abnormality_score[:, 0].cpu()

            mask = mask.reshape(-1)
            score = (1. / normality_score).unsqueeze(1).repeat(1, mask.shape[0])
    ...
    def calculate_visual_anomaly_score(self, visual_features):
        ...
        for indx, (features, mask) in enumerate(zip(visual_features, self.masks)):
            normality_score = 0.5 * (1 - (features.float() @ cur_visual_gallery.float().T).max(dim=1)[0])
            normality_score = normality_score.cpu()

            mask = mask.reshape(-1)
            score = normality_score.unsqueeze(1).repeat(1, mask.shape[0])
